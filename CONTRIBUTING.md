...
External tools (e.g., the SMP template) link into the guide through stable
`/go/<slug>/` URLs, so their links do not break when a heading is renamed or a
page moves. Those URLs are defined in [`permalinks.yml`](permalinks.yml) and
generated by `hooks/permalinks.py`. When a linked section changes
its title or location, update that section's `target` in `permalinks.yml`, so the
external link never changes. `mkdocs build --strict` fails if a target no longer
resolves.
